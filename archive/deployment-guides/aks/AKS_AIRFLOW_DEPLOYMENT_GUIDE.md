...
**Note on Azure Storage for Logs (Optional):**
Remote log storage in Azure Blob is optional and not required for this POC. If needed later, create secrets via kubectl in your terminal only—never commit credentials to Git. See Azure Key Vault integration for production secret management.
